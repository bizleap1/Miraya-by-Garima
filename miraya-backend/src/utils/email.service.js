...
const FROM_EMAIL = process.env.EMAIL_FROM || 'Miraya By Garima <orders@mirayabygarima.com>';

/**
 * Send email using Resend with verified domain (mirayabygarima.com)
 */
...
export const sendWelcomeEmail = async (email, name) => {
  return sendEmail({
    to: email,
    subject: '✨ Welcome to Miraya By Garima',
    html: `
      <div style="font-family: 'Helvetica', sans-serif; max-width: 500px; margin: 0 auto; background: #ffffff; border: 1px solid #c6a46a; border-radius: 12px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.08);">
        <div style="background: #5e0a0b; color: #ffffff; padding: 25px; text-align: center;">
          <h2 style="font-family: serif; letter-spacing: 2px; margin: 0; font-size: 20px; color: #ffffff;">MIRAYA BY GARIMA</h2>
        ...
        <div style="padding: 30px; text-align: center; color: #333333;">
          ...
          <p style="color: #666; font-size: 14px; line-height: 1.6;">Thank you for registering with <strong>Miraya By Garima</strong> — luxury handcrafted ethnic wear.</p>
...
export const sendOrderConfirmationEmail = async (email, order, pdfBuffer = null) => {
  const attachments = pdfBuffer ? [
    {
      filename: `Miraya_Invoice_#MRY-${order.id}.pdf`,
      content: pdfBuffer
    }
  ] : [];
  ...
  return sendEmail({
    to: email,
    subject: isCod
      ? `📦 COD Order Confirmed #MRY-${order.id} - Miraya By Garima`
      : `✨ Order Confirmed #MRY-${order.id} - Miraya By Garima`,
    attachments,
    html: `
      <div style="font-family: 'Helvetica', sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff; border: 1px solid #c6a46a; border-radius: 12px; overflow: hidden; box-shadow: 0 8px 25px rgba(0,0,0,0.08);">
        <div style="background: #5e0a0b; color: #ffffff; padding: 25px; text-align: center;">
          <h2 style="font-family: serif; letter-spacing: 2px; margin: 0;">MIRAYA BY GARIMA</h2>
        ...
        <div style="padding: 25px; color: #333333; line-height: 1.6;">
          ...
          <p>Your order <strong>#MRY-${order.id}</strong> has been successfully placed and is being prepared for dispatch.</p>
          
          <div style="background: #f8f5f0; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #5e0a0b;">
            <p style="margin: 0 0 5px 0;"><strong>Order Reference:</strong> #MRY-${order.id}</p>
          ...
          <p style="margin-top: 30px; font-size: 12px; color: #777; text-align: center; border-top: 1px solid #eee; padding-top: 15px;">
            If you have any questions, reply directly to this email or contact us at <a href="mailto:orders@mirayabygarima.com" style="color: #5e0a0b;">orders@mirayabygarima.com</a>.
          </p>
        </div>
      </div>
    `,
...
export const sendCancellationRequestEmail = async (email, orderId, reason) => {
  return sendEmail({
    to: email,
    subject: `Order Cancellation Request Submitted - #MRY-${orderId}`,
    html: `
      <div style="font-family: sans-serif; padding: 20px; background: #f8f5f0;">
        <h2 style="color: #5e0a0b;">Cancellation Request Received</h2>
        <p>Your cancellation request for order <strong>#MRY-${orderId}</strong> has been received.</p>
  ...
  return sendEmail({
    ...
    html: `
      <div style="font-family: sans-serif; padding: 20px; background: #ffffff; border: 1px solid #c6a46a; border-radius: 10px;">
        ...
        <p>Your request to cancel order <strong>#MRY-${orderId}</strong> has been <strong>${isAccepted ? 'ACCEPTED' : 'REJECTED'}</strong> by our admin team.</p>
...
export const sendOrderStatusUpdateEmail = async (email, orderId, status) => {
  return sendEmail({
    to: email,
    subject: `Order #MRY-${orderId} Status Update: ${status}`,
    html: `
      <div style="font-family: sans-serif; padding: 20px; background: #ffffff; border: 1px solid #5e0a0b; border-radius: 10px;">
        ...
        <p>The status of your order <strong>#MRY-${orderId}</strong> has been updated to: <strong style="color: #c6a46a;">${status.toUpperCase()}</strong>.</p>
      </div>
    `,
  });
};
...
export const sendPasswordResetOtpEmail = async (email, otp) => {
  return sendEmail({
    to: email,
    subject: '🔑 Password Reset OTP - Miraya By Garima',
    html: `
      <div style="font-family: 'Helvetica', sans-serif; max-width: 500px; margin: 0 auto; background: #ffffff; border: 1px solid #c6a46a; border-radius: 12px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.08);">
        <div style="background: #5e0a0b; color: #ffffff; padding: 25px; text-align: center;">
          <h2 style="font-family: serif; letter-spacing: 2px; margin: 0; font-size: 20px; color: #ffffff;">MIRAYA BY GARIMA</h2>
...
export const sendLoginOtpEmail = async (email, otp) => {
  return sendEmail({
    to: email,
    subject: '🔑 Your Login OTP - Miraya By Garima',
    html: `
      <div style="font-family: 'Helvetica', sans-serif; max-width: 500px; margin: 0 auto; background: #ffffff; border: 1px solid #c6a46a; border-radius: 12px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.08);">
        <div style="background: #5e0a0b; color: #ffffff; padding: 25px; text-align: center;">
          <h2 style="font-family: serif; letter-spacing: 2px; margin: 0; font-size: 20px;">MIRAYA BY GARIMA</h2>
...
export const sendRegisterOtpEmail = async (email, otp, name = 'Client') => {
  return sendEmail({
    to: email,
    subject: '✨ Account Registration OTP - Miraya By Garima',
    html: `
      <div style="font-family: 'Helvetica', sans-serif; max-width: 500px; margin: 0 auto; background: #ffffff; border: 1px solid #c6a46a; border-radius: 12px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.08);">
        <div style="background: #5e0a0b; color: #ffffff; padding: 25px; text-align: center;">
          <h2 style="font-family: serif; letter-spacing: 2px; margin: 0; font-size: 20px;">MIRAYA BY GARIMA</h2>
        ...
        <div style="padding: 30px; text-align: center; color: #333333;">
          ...
          <p style="color: #666; font-size: 14px;">Your One-Time Password (OTP) to complete your registration with Miraya By Garima is:</p>
